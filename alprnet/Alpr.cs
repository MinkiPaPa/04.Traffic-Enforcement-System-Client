...
namespace AlprNet
{
    public class Alpr : IDisposable
    {
	    ...
	    public Alpr(string country, string config_file = "", string runtime_dir = "", string license_key = "", int hardware_acceleration = 0, int gpu_id = 0, int batch_size = 1)
        {
        ...
        public bool Initialize()
        {
            ...
            {
                Console.WriteLine("Could not find/load native library (libopenalpr.dll)");
                return false;
            }
        ...
        public AlprResponse Recognize(System.Drawing.Image image)
        {
            ...
            System.Drawing.Bitmap clone = new System.Drawing.Bitmap(image.Width, image.Height, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
        ...
        public AlprResponse[] RecognizeBatch(string[] filepath)
        {
            List<byte[]> bytes = new List<byte[]>();
            foreach (string file in filepath)
            {
                bytes.Add(System.IO.File.ReadAllBytes(file));
            }
            return RecognizeBatch(bytes);
        }

        public AlprResponse[] RecognizeBatch(List<byte[]> images_bytes)
        {
            check_initialization();
            IntPtr native_batch = openalpr_create_image_batch();
            int images = 0;

            foreach (byte[] image in images_bytes)
            {
                NativeROI roi;
                roi.x = 0;
                roi.y = 0;
                ...
                IntPtr unmanagedArray = Marshal.AllocHGlobal(image.Length);
                Marshal.Copy(image, 0, unmanagedArray, image.Length);

                openalpr_add_encoded_image_to_batch(native_batch, unmanagedArray, image.Length, roi);
                images++;
            }

            IntPtr resp_ptr = openalpr_recognize_batch(this.native_instance, native_batch);
            ...
            AlprResponse[] response = JsonConvert.DeserializeObject<AlprResponse[]>(utf8_json);
            openalpr_release_image_batch(native_batch);
            openalpr_free_response_string(resp_ptr);
