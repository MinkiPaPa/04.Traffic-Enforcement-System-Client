    ...
    public partial class FrmMain : Form
    {
        ...
        private void Fn_SetCodeWithDB()
        {
            try
            {
                ...
                // 사용자 업무 Role에 따른 사용자 level
                try
                {
                    int rv = iTopsLib.Lib.GFn_GetPermissionLevelDS(ref dsLevel);
                    if (rv < 0) return;
                    if (rv > 0)
                    {
                        CbbUserLevel.DataSource = dsLevel.Tables[0];

                        CbbUserLevel.DisplayMember = "role_nm";
                        CbbUserLevel.ValueMember = "level";

                        CbbUserLevel.Text = null;
                    }
        ...
        {
            txtPrsnId.ValueChanged -= DetailItem_ValueChanged;
            txtName.ValueChanged -= DetailItem_ValueChanged;
            txtMobile.ValueChanged -= DetailItem_ValueChanged;
            txtPhone.ValueChanged -= DetailItem_ValueChanged;
            txtEMail.ValueChanged -= DetailItem_ValueChanged;
            txtAddr.ValueChanged -= DetailItem_ValueChanged;

            chkUser.CheckedChanged -= DetailItem_ValueChanged;

            txtAccountId.ValueChanged -= DetailItem_ValueChanged;
            txtPassWord.ValueChanged -= DetailItem_ValueChanged;
            CbbUserLevel.ValueChanged -= DetailItem_ValueChanged;
            chkAllowed.CheckedChanged -= DetailItem_ValueChanged;
            txtFailCnt.ValueChanged -= DetailItem_ValueChanged;
            ...
            try
            {
                ...
                // 값 넣기
                txtPrsnId.Value = Convert.ToString(DBGrid.Rows[index].Cells["prsn_id"].Value);
                txtName.Value = Convert.ToString(DBGrid.Rows[index].Cells["name"].Value);
                txtMobile.Value = Convert.ToString(DBGrid.Rows[index].Cells["mobile"].Value);
                txtPhone.Value = Convert.ToString(DBGrid.Rows[index].Cells["phone"].Value);
                txtEMail.Value = Convert.ToString(DBGrid.Rows[index].Cells["email"].Value);
                txtAddr.Value = Convert.ToString(DBGrid.Rows[index].Cells["addr"].Value);

                chkUser.Checked = Convert.ToBoolean(DBGrid.Rows[index].Cells["user_yn"].Value);

                txtAccountId.Value = Convert.ToString(DBGrid.Rows[index].Cells["user_id"].Value);
                txtPassWord.Value = Convert.ToString(DBGrid.Rows[index].Cells["user_pw"].Value);
                CbbUserLevel.Value = Convert.ToString(DBGrid.Rows[index].Cells["level"].Value);

                chkAllowed.Checked = Convert.ToBoolean(DBGrid.Rows[index].Cells["allowed_yn"].Value);

                txtFailCnt.Value = Convert.ToString(DBGrid.Rows[index].Cells["fail_cnt"].Value);

                // ValueChanged EventHandler를 같이 사용하기 위함
                txtPrsnId.Tag = Convert.ToString(DBGrid.Rows[index].Cells["prsn_id"].Column.Index);
                txtName.Tag = Convert.ToString(DBGrid.Rows[index].Cells["name"].Column.Index);
                txtMobile.Tag = Convert.ToString(DBGrid.Rows[index].Cells["mobile"].Column.Index);
                txtPhone.Tag = Convert.ToString(DBGrid.Rows[index].Cells["phone"].Column.Index);
                txtEMail.Tag = Convert.ToString(DBGrid.Rows[index].Cells["email"].Column.Index);
                txtAddr.Tag = Convert.ToString(DBGrid.Rows[index].Cells["addr"].Column.Index);

                chkUser.Tag = Convert.ToString(DBGrid.Rows[index].Cells["user_yn"].Column.Index);

                txtAccountId.Tag = Convert.ToString(DBGrid.Rows[index].Cells["user_id"].Column.Index);
                txtPassWord.Tag = Convert.ToString(DBGrid.Rows[index].Cells["user_pw"].Column.Index);
                CbbUserLevel.Tag = Convert.ToString(DBGrid.Rows[index].Cells["level"].Column.Index);

                chkAllowed.Tag = Convert.ToString(DBGrid.Rows[index].Cells["allowed_yn"].Column.Index);

                txtFailCnt.Tag = Convert.ToString(DBGrid.Rows[index].Cells["fail_cnt"].Column.Index);

                // 수정인 경우 Resident Number, Account 수정 못하게
                txtPrsnId.Enabled = (Convert.ToString(DBGrid.Rows[index].Cells["prsn_id_org"].Value) == String.Empty);
                txtAccountId.Enabled = (Convert.ToString(DBGrid.Rows[index].Cells["user_id_org"].Value) == String.Empty);
            ...
            finally
            {
                ...
                // 계정이 존재여부에 따라 보여주거나 숨김
                grpbxAccount.Visible = chkUser.Checked;
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (UltraGridRow row in DBGrid.Rows)
                {
                    ...
                    strArrRoleTP[iPos] = "RLT0000" + Convert.ToString(row.Cells["level"].Value);
        ...
        private void BtnCancel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    txtPrsnId.Value = Convert.ToString(DBGrid.ActiveRow.Cells["prsn_id_org"].Value);
                    txtName.Value = Convert.ToString(DBGrid.ActiveRow.Cells["name_org"].Value);
                    txtMobile.Value = Convert.ToString(DBGrid.ActiveRow.Cells["mobile_org"].Value);
                    txtPhone.Value = Convert.ToString(DBGrid.ActiveRow.Cells["phone_org"].Value);
                    txtEMail.Value = Convert.ToString(DBGrid.ActiveRow.Cells["email_org"].Value);
                    txtAddr.Value = Convert.ToString(DBGrid.ActiveRow.Cells["addr_org"].Value);

                    chkUser.Checked = Convert.ToBoolean(DBGrid.ActiveRow.Cells["user_yn_org"].Value);

                    txtAccountId.Value = Convert.ToString(DBGrid.ActiveRow.Cells["user_id_org"].Value);
                    txtPassWord.Value = Convert.ToString(DBGrid.ActiveRow.Cells["user_pw_org"].Value);
                    CbbUserLevel.Value = Convert.ToString(DBGrid.ActiveRow.Cells["level_org"].Value);

                    chkAllowed.Checked = Convert.ToBoolean(DBGrid.ActiveRow.Cells["allowed_yn_org"].Value);

                    txtFailCnt.Value = Convert.ToString(DBGrid.ActiveRow.Cells["fail_cnt_org"].Value);
        ...
        private void ComboBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if ((e.KeyChar >= ' ' && e.KeyChar <= '~'))
            {
                ...
                try
                {
                    ...
                    {
                        DataRow[] row = dsLevel.Tables[0].Select(string.Format("level like '*{0}*'", strInput));
                        if (row.Length == 0)
                        {
                            row = dsLevel.Tables[0].Select(string.Format("role_nm like '*{0}*'", strInput));
                            if (row.Length == 0)
                            {
                                e.KeyChar = (char)0;    // 입력 취소
                                Lib.Beep(1000, 20);     // Beep
